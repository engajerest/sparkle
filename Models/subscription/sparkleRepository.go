...
const (
	...
	getsubscription                = "SELECT a.packageid,a.moduleid,a.tenantid,a.totalamount,b.modulename,b.logourl,b.iconurl,'' as packagename,'0.0' AS packageamount,'' as packageicon, (SELECT COUNT(locationid)  FROM tenantlocations where  tenantid =?) AS location, (SELECT COUNT(tenantcustomerid)  FROM tenantcustomers WHERE tenantid =?) AS customer FROM tenantsubscription a , app_module b WHERE a.moduleid=b.moduleid AND   a.tenantid=?"
...
func GetAllSubscription(tenantid int) []Subscribe {
	...
	for rows.Next() {
		var s Subscribe
		err := rows.Scan(&s.Packageid, &s.Moduleid, &s.Tenantid, &s.Totalamount, &s.Modulename, &s.Logourl,&s.Iconurl, &s.Packagename,
